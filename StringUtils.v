...
Proof.
 split.
 - revert s'.
   induction s; destruct s'; simpl; auto.
   + intros [=].
   + destruct Ascii.ascii_dec; subst; auto. intros [=].
 - induction 1.
   + now destruct s.
...
Proof.
 ...
 - intros [|a1 s1] s2; simpl in *.
   + split.
     * inversion_clear 1. right. exists (String a s); auto.
     * intros [H|(s' & -> & H)]; auto. inversion H.
   + split.
     * inversion_clear 1. apply IH in H0.
       destruct H0 as [H0|(s' & <- & H0)]; [left|right]; auto.
       exists s'; auto.
     * intros [H|(s' & H & H')]; auto.
