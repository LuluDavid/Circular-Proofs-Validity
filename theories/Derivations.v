...
Definition valid_deriv_step '(ORule ls R s ld) :=
  ...
  | [(⊢ Γ')], [ls'] => valid_next_sequent_unary ls' ls s &&&
                       ...
                       | Mu, (⊢ { µ k F, a }::Γ)     => (Γ' =? {F[[ %0 := µ k F ]], i::a}::Γ)
                       | Nu, (⊢ { ν k F, a }::Γ)     => (Γ' =? {F[[ %0 := ν k F ]], i::a}::Γ)
                       | _, _                        => false
                       end
...
(* Necessary to have an instantiation of F here, 
   as dual will not be computable for a generic formula *)
Definition oderiv_example G :=
  ORule [] Or_mult (⊢[{ (G #(dual G)), [] }]) [ORule [] Ax (⊢[{ G, [l] }; {dual G, [r]}]) [] ].

Compute oderiv_example example1.
...
Compute valid_deriv (oderiv_example example1).
...
Inductive preValid : derivation -> Prop :=
 ...
 | V_Ex d F G Γ1 Γ2 ls : preValid d 
                          -> Claim d (⊢ app Γ1 (G::F::Γ2)) 
                          -> Backedges d (lift ls) \/ Backedges d ( (1, ⊢app Γ1 (F::G::Γ2)) :: lift ls)
                          -> preValid (ORule ls Ex (⊢app Γ1 (F::G::Γ2)) [d])
 | V_Mu d F Γ ls a k : preValid d 
                        -> Claim d (⊢ { F[[ %0 := µ k F ]], i::a }::Γ) 
                        -> Backedges d (lift ls) \/ Backedges d ( (1, ⊢ { (µ k F), a }::Γ) :: lift ls)
                        -> preValid (ORule ls Mu (⊢ { (µ k F), a }::Γ) [d])
 | V_Nu d F Γ ls a k : preValid d 
                        -> Claim d (⊢ {F[[ %0 := ν k F ]], i::a}::Γ) 
                        -> Backedges d (lift ls) \/ Backedges d ( (1, ⊢ { (ν k F), a }::Γ) :: lift ls)
                        -> preValid (ORule ls Nu (⊢ { (ν k F), a }::Γ) [d])

 | V_BackEdge ls s n: option_oseq_equiv (Some s) (list_assoc n ls) 
                      -> preValid (ORule ls (BackEdge n) s [])
 .

Hint Constructors preValid : circular.
...
ORule [] Nu (⊢ [{ (ν 0 (%0⊕%0)), [] }]) 
          [
          (
          ORule [(1, ⊢ [{ (ν 0 (%0⊕%0)), [] }])] Or_add_l (⊢ [ { ((ν 0 (%0⊕%0)) ⊕ (ν 0 (%0⊕%0))), [i] } ] )
            [
            (
            ORule [(2, ⊢ [{ (ν 0 (%0⊕%0)), [] }])] (BackEdge 2) (⊢ [ { ν 0 (%0⊕%0), [l;i] } ] ) []
            )
            ] 
          ) 
          ].
...
Proof.
  unfold oderiv_example'.
  repeat (constructor; auto with string circular).
Qed.
...
Compute (valid_deriv (cut_example (% 0))).



(** Printing functions *)

Definition context_example : context := [ν 0 (%0⊕%0); µ 0 (%1#%0)].
Compute (print_ctx context_example).

Definition sequent_example := Seq context_example.
Compute (print_seq sequent_example).

Definition print_list_oseq (ls:list (nat*osequent)) :=
  match ls with
  | [] => ""
  | (n, s)::ls => (print_oseq s)
  end.
...
Proof.
 ...
 - induction d as [ls r s ds IH] using derivation_ind'.
   ...
   + apply swapb_is_swap in H1; do 5 destruct H1; subst; econstructor; mytac;
         rewrite H2; trivial.
   + mytac; apply (V_Cut _ _ F0 l1 l4 _ a a0); mytac; apply disjoint_addr_list_is_disjoint_addr_listb; trivial.
 - induction 1; simpl; trivial;
   ...
   + rewr; repeat rewrite lazy_andb_iff; split; auto.
    ...
    + rewr; rewrite lazy_andb_iff; split; auto; destruct (Γ1 ++ G :: F0 :: Γ2) eqn:Heq; 
