...
Class OBSubst (A : Type) := obsubst : Atom -> Occurrence -> A -> A.
Arguments obsubst {_} {_} _ _ !_.
Hint Extern 1 (OBSubst _ _ _) => cbn : circular.
...
Instance occ_bsubst: BSubst Occurrence :=
  fix occ_obsubst At f o := 
  let '{ F, A} := o in ({  F[[ %At := f ]], A }).
...
Definition occ_dual (o:Occurrence): Occurrence :=
  let '(Occ F a) := o in
  { (dual F), (addr_dual a)}.
...
Definition ocontext_forget Γ : list formula := map occ_forget Γ.

Definition ocontext_addr Γ : list address := map occ_addr Γ.
...
Definition oseq_forget (s:osequent): sequent := (⊦ (ocontext_forget s)).

Definition oseq_addr (s:osequent): list address := ocontext_addr s.
...
Definition octx_example := [{ (µ 0 ((% 0)&(!#(% 0)))), [i;l;r] }
                            ;{ (ν 0 (µ 1 ((% 1)&(!#(% 0))))), [] }].
...
Definition combine_eq_length (l l' : list derivation) := 
...
Arguments Claim _ _ /.
Hint Extern 1 (Claim _ _) => cbn : circular.

Definition Backedges d ls := backedges d = ls.
Arguments Backedges _ _ /.
Hint Extern 1 (Backedges _ _) => cbn : circular.
