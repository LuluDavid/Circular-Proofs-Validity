...
| SFQuant (F G:formula)(a:Atom)(q:quant): Subform F G -> Subform (Quant q a F) G
.

Hint Constructors Subform : circular.
...
Proof.
  ...
  try(apply orb_true_iff; apply orb_true_iff in H; destruct H;
            try(left; eapply (IHF1 G _); try assumption; try eapply (IHF1 (Quant q a G) _); try assumption;
            simpl; rewrite orb_true_iff; right; apply subform_b_refl);
            try(right; eapply (IHF2 G _); try assumption; try eapply (IHF2 (Quant q a G) _); try assumption;
               simpl; rewrite orb_true_iff; right; apply subform_b_refl);
               reflexivity);
  ...
  eapply (IHF G _); try assumption; eapply (IHF (Quant q0 a0 G) _); try assumption;
  ...
  - repeat rewrite orb_true_iff; simpl in H1; simpl in H2; 
   ...
   - apply orb_true_iff; repeat rewrite orb_true_iff in H0; simpl in H1; repeat rewrite orb_true_iff in H1.
    ...
    - simpl in H1; simpl in H2; apply orb_true_iff in H1; repeat rewrite orb_true_iff.
      destruct H1.
      -- right; left; eapply (IHF1 G _); try assumption; eapply (IHF1 (Quant q a G) _); try assumption;
         simpl; apply orb_true_iff; right; apply subform_b_refl.
      -- right; right; eapply (IHF2 G _); try assumption; eapply (IHF2 (Quant q a G) _); try assumption;
         simpl; apply orb_true_iff; right; apply subform_b_refl.
    - simpl in H1; simpl in H2; apply orb_true_iff in H0; apply orb_true_iff in H1; repeat rewrite orb_true_iff;
      ...
      destruct H0.
      -- left; eapply (IHF1 G _); try assumption; eapply (IHF1 (Quant q a G) _); try assumption;
         simpl; apply orb_true_iff; right; apply subform_b_refl.
      -- right; eapply (IHF2 G _); try assumption; eapply (IHF2 (Quant q a G) _); try assumption;
         simpl; apply orb_true_iff; right; apply subform_b_refl.
...
- apply orb_true_iff; apply orb_true_iff in H0; simpl in H1; repeat rewrite orb_true_iff in H1;
  ...
  -- right; subst; assumption.
  -- right; eapply (IHF G); try assumption; eapply (IHF (Quant q0 a0 G)); try assumption;
     simpl; rewrite orb_true_iff; right; apply subform_b_refl.
Qed.
...
Proof.
intros. split; intros. 
  + induction H as [F | F G F' o IH IH'| F G F' o IH IH'| F G q a H IH]; trivial;
  ...
  + induction F as [ | | | | V | o F1 IH1 F2 IH2 | q a F IH]; 
     induction G as [ | | | | V' | o' G1 IH1' G2 IH2' | q' a' F' IH'];
    ...
    - apply andb_true_iff in H; destruct H; apply andb_true_iff in H; destruct H; apply Utils.eqb_eq in H; apply Utils.eqb_eq in H1;
apply Utils.eqb_eq in H0; subst; apply SRefl.
...
Lemma subform_quant: forall F a q, ~ (Quant q a F ⧼ F).
Proof.
  intros; unfold not; intro; apply subform_size in H; simpl in H; lia.
Qed.
...
Proof.
  assert (Hloc: forall x x' o, (x ⧼ Op o x x') /\ (x' ⧼ Op o x x')). { eauto with circular. }
  assert (HlocBis: forall x a q, (x ⧼ Quant a q x)). { auto with circular. }
 ...
 - rewrite subform_b_is_subform in *; simpl in *. rewrite orb_true_iff in *.
   ...
   + rewrite <- subform_b_is_subform in *.
      ...
      destruct (subform_quant y a q); trivial.
Qed.
