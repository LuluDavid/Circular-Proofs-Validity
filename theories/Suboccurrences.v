...
Inductive PreSuboccurrence: Occurrence -> Occurrence -> Prop :=
  ...
  | PUn (F:formula)(q:quant)(a:Atom)(A:address): PreSuboccurrence { (Quant q a F), A }  { F, i::A}
  .
...
Inductive Suboccurrence: Occurrence -> Occurrence -> Prop :=
  ...
  | Un (F:formula)(q:quant)(a:Atom)(A:address): Suboccurrence { (Quant q a F), A }  { F, i::A}
  | Refl (F:Occurrence): Suboccurrence F F
  | Trans (F G H:Occurrence): Suboccurrence F G -> Suboccurrence G H -> Suboccurrence F H
  .
Hint Constructors Suboccurrence : circular.
...
Fixpoint suboccurrence_b_rec ( F:formula )(A:address)(G:Occurrence){ struct F } := 
  ...
  | (Quant q a F'), G => (suboccurrence_b_rec F' (i::A) G)
  | _, _ => false
  end.
...
Definition f1 : Occurrence := { %0, [l; i; i] }.
Definition f2 : Occurrence := { µ 0 (%0 & ⊥), [i] }.
...
Proof.
  ...
  apply orb_true_iff; apply orb_true_iff in H; destruct H; try (right; apply (IHF a0 (i :: a1) {F1, a2}); assumption);
    try(apply eqb_eq in H; injection H as H1; subst; simpl in H0; apply orb_true_iff in H0; destruct H0;
